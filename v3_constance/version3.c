...
#define VITESSE 100000 // Vitesse d'avancement du serpent
...
void progresser(int lesX[TAILLE], int lesY[TAILLE], char direction, bool *collisions); // la procédure calcule et affiche la prochaine position du serpent
...
void desssinerPlateau(airDeJeu plateau);
...
int main()
{
        ...
        disableEcho();
        while ((saisie != ARRET) && (collisions != true))
        {
                progresser(lesX, lesY, saisie, &collisions);
                if (kbhit() == 1)
                {
                        nvSaisie = getchar();
...
void progresser(int lesX[TAILLE], int lesY[TAILLE], char direction, bool *collisions)
{
        int i;
...
void initPlateau(airDeJeu plateau)
{
        for (int lig = 0; lig <= X; lig++)
        {
                for (int col = 0; col <= Y; col++)
                {
                        if (((lig == 1) || (lig == X)) || ((col == 1) || (col == Y)))
                        {
                                plateau[lig][col] = '#';
                        }
                        else
